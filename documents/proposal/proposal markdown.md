...
### 2.3.1 Evaluating Ensemble Versus Non-Ensemble Machine Learning Performance with Preprocessing Techniques for IoT Intrusion Detection on CICIoT2023 [10]

Firdaus et al. [10] mengevaluasi performa model *ensemble* dibandingkan non-*ensemble* dengan penekanan pada peran teknik *preprocessing* dalam meningkatkan stabilitas dan konsistensi model pada CICIoT2023. Penelitian ini menerapkan *random undersampling* untuk mengatasi ketidakseimbangan kelas yang ekstrem, dikombinasikan dengan seleksi fitur hibrida yang memadukan *Mutual Information* (MI) dan *Random Forest Feature Importance* (RFFI). Lima model diuji — Naive Bayes, Logistic Regression, SVM, Random Forest, dan XGBoost — menggunakan *stratified 5-fold cross-validation* untuk menghasilkan estimasi performa yang lebih stabil. XGBoost tampil sebagai model terbaik dengan *macro F1-score* rata-rata 0,8891 ± 0,0008. Kontribusi penting penelitian ini adalah pembuktian kuantitatif bahwa normalisasi bukan langkah opsional: Logistic Regression tanpa *scaling* menghasilkan F1-score tidak stabil sebesar 0,6280, namun melonjak menjadi 0,7691 setelah normalisasi Min-Max diterapkan — temuan yang memperkuat pentingnya konsistensi *preprocessing* dalam pipeline deteksi intrusi. Meskipun demikian, seluruh eksperimen dilakukan dalam satu domain dataset yang sama, sehingga robustness pipeline maupun model terhadap data dari domain IIoT industri yang berbeda distribusinya belum terbukti.

### 2.3.2 Accelerating Classification for IoT Attack Detection Using Decision Tree Model with Gini Impurity Tree-Based Feature Selection Technique [11]

Dzaki et al. [11] mengembangkan pendekatan akselerasi klasifikasi serangan IoT berbasis seleksi fitur *Gini Impurity Tree-based* pada CICIoT2023. Penelitian ini membandingkan enam teknik seleksi fitur dari tiga kategori — *filter* (Fisher Score, Mutual Information), *wrapper* (Recursive Feature Elimination, Genetic Algorithm), dan *embedded* (Gini Impurity Tree-based, Lasso-based) — yang dievaluasi pada Decision Tree dengan *split* 9:1 dan *resampling* kombinasi *random under-* dan *oversampling* ke 437.853 sampel per kelas. *Gini Impurity Tree-based* terbukti paling efisien: menghasilkan subset 10 fitur dengan waktu pelatihan hanya 40 detik — 63,06% lebih cepat dari *baseline* tanpa seleksi fitur (148 detik) — menghasilkan pohon keputusan lebih dangkal (64 level dari 73 level *baseline*) — dengan akurasi 0,994 dan Macro F1 0,839. Penelitian ini memfokuskan kontribusinya pada efisiensi komputasi dalam satu dataset; tidak dikaji apakah subset 10 fitur yang terpilih bersifat *robust* ketika diterapkan pada domain dataset yang berbeda karakteristik distribusinya.

### 2.3.3 Interpretable Intrusion Detection for IoT: A CNN-BiLSTM Permutation Importance Framework for Deep Feature Selection [22]

Al-Shibly et al. [22] merupakan satu-satunya penelitian yang secara eksplisit menggunakan DataSense (CIC IIoT 2025) sebagai objek eksperimen. Penelitian ini mengajukan kerangka *deep feature selection* (DFS) berbasis *hybrid* CNN-BiLSTM yang menggabungkan kemampuan CNN dalam menangkap interaksi fitur lokal dan BiLSTM dalam menangkap dependensi temporal dua arah, dilengkapi penilaian kepentingan fitur menggunakan *permutation importance*. Dari 94 fitur asli DataSense, 20 fitur terpilih kemudian digunakan untuk melatih *classifier* ringan — Decision Tree, KNN, Logistic Regression, Naive Bayes, dan Random Forest. Eksperimen menggunakan pembagian 70:30 *stratified* pada 685.671 instans DataSense. Hasil terbaik dicapai oleh Random Forest dengan akurasi 0,9720 dan F1-score 0,9719, diikuti Decision Tree dengan akurasi 0,9710 dan F1-score 0,9708. Terdapat dua keterbatasan mendasar: penelitian ini hanya menangani *binary classification* (benign vs. attack) tanpa membedakan jenis serangan secara multiclass, dan seluruh evaluasi dilakukan *in-dataset* pada DataSense tanpa keterlibatan dataset sumber dari domain lain. Al-Shibly et al. [22] sendiri mengakui bahwa evaluasi multiclass "would offer deeper insights into the various types of attacks" dan hasil penelitian "are confined to this particular data set" — mengindikasikan kebutuhan evaluasi multiclass lintas domain yang belum terpenuhi.

### 2.3.4 Cross-Dataset Temporal and Semantic Generalization of Intrusion Detection Models for the Future Internet [29]

Elangovan et al. [29] menyajikan studi generalisasi lintas dataset yang sistematis, mengevaluasi lima model — Random Forest, Gradient Boosting, MLP, Autoencoder, dan 1D-CNN — pada empat dataset dari rentang 2017 hingga 2024 (CIC-IDS2017, CSE-CIC-IDS2018, CICIoT2023, CIC-DIAD2024) tanpa *retraining*. Dalam evaluasi *in-dataset*, seluruh model mencapai Macro-F1 antara 0,84 dan 0,96; namun pada evaluasi temporal maju, model yang dilatih pada trafik *enterprise* 2017 mengalami degradasi ΔF1 ≈ 0,20–0,27 ketika diuji pada dataset IoT 2023–2024. Pada evaluasi lintas domain (*enterprise* → IoT), Macro-F1 turun ke rentang 0,69–0,78 dan *false positive rate* trafik benign meningkat hingga 0,30. Dua temuan kritis penelitian ini langsung relevan bagi penelitian ini: (1) model berbasis *tree-ensemble* menunjukkan degradasi paling rendah (≈6–23%) dengan varians antar-dataset yang lebih kecil dibandingkan model *neural* maupun *representation learning*; dan (2) fitur yang mencerminkan *Flow Intensity* dan *Temporal Activity* memiliki stabilitas lintas dataset yang lebih tinggi dibandingkan *Protocol-Level Indicators*. Meskipun demikian, penelitian Elangovan et al. [29] bergerak dari arah *enterprise* ke IoT — kebalikan dari arah yang relevan bagi penelitian ini — dan tidak mencakup pasangan CICIoT2023→DataSense secara spesifik, sehingga *generalization gap* pada arah IoT konsumen → IIoT industri untuk pasangan dataset tersebut belum pernah diukur.

Secara keseluruhan, keempat penelitian di atas meninggalkan celah yang belum tertangani secara langsung. Firdaus et al. [10] dan Dzaki et al. [11] membuktikan bahwa model berbasis CICIoT2023 dapat mencapai performa kompetitif secara *in-dataset*, namun tidak menguji ketahanan model maupun fitur yang dihasilkan di luar distribusi dataset asalnya. Al-Shibly et al. [22] menjadi satu-satunya rujukan yang menggunakan DataSense, namun evaluasinya terbatas pada klasifikasi biner *in-dataset*. Elangovan et al. [29] membuktikan secara empiris bahwa degradasi lintas dataset nyata dan signifikan serta mengonfirmasi keunggulan *tree-ensemble* dalam ketahanan distribusi, namun tidak mengukur pasangan CICIoT2023→DataSense secara langsung. Tidak satu pun dari keempat penelitian ini yang mengukur *generalization gap* antara CICIoT2023 (IoT konsumen) dan DataSense (IIoT industri) dalam skenario multiclass *cross-dataset* — celah yang menjadi motivasi utama penelitian ini.

Tabel 2.1 Gap penelitian

| Ref  | Peneliti           | Tahun | Dataset                                                         | Kelas      | Metode                                                   | Hasil Terbaik                      | Gap                                                                                                         |
| ---- | ------------------ | ----- | --------------------------------------------------------------- | ---------- | -------------------------------------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| [10] | Firdaus et al.     | 2026  | CICIoT2023                                                      | Multiclass | Ensemble vs non-ensemble, MI+RFFI FS, undersampling      | XGBoost: Macro F1 0,8891           | In-dataset only; pipeline tidak divalidasi di domain IIoT industri                                         |
| [11] | Dzaki et al.       | 2025  | CICIoT2023                                                      | Multiclass | DT + Gini Impurity Tree-based FS                         | DT: Accuracy 0,994, Macro F1 0,839 | In-dataset only; 10 fitur terpilih tidak divalidasi lintas domain                                           |
| [22] | Al-Shibly et al.   | 2026  | CIC IIoT 2025                                                   | **Binary** | CNN-BiLSTM + Permutation Importance                      | RF: F1 0,9719                      | Binary only; in-dataset DataSense only; tidak ada evaluasi multiclass atau lintas dataset                   |
| [29] | Elangovan et al.   | 2026  | CIC-IDS2017, CSE-CIC-IDS2018, CICIoT2023, CIC-DIAD2024         | Multiclass | RF, Gradient Boosting, MLP, Autoencoder, 1D-CNN          | Cross-domain Macro-F1 0,69–0,78    | Arah enterprise→IoT; tidak mencakup pasangan CICIoT2023→DataSense; gap IoT→IIoT belum terukur              |
...
Pipeline CICIoT2023 terdiri dari empat tahap berurutan: (1) *data cleaning* dan *random undersampling* untuk membersihkan dan menyeimbangkan distribusi kelas [10][11]; (2) *Gini Impurity Tree-based feature selection* untuk mereduksi 48 fitur menjadi 10 fitur terpilih [11]; (3) normalisasi Min-Max ke rentang [0,1] menggunakan parameter yang dihitung dari data CICIoT2023 [10][11]; dan (4) pembagian *stratified* 80:20 untuk pelatihan dan evaluasi *in-dataset*. Pipeline DataSense mengikuti tahap yang paralel — *cleaning*, *feature alignment* ke 10 fitur yang sama, dan normalisasi dengan parameter identik dari CICIoT2023 — sehingga seluruh DataSense berfungsi sebagai *test set* evaluasi *cross-dataset*. Label diselaraskan ke lima kelas bersama: DDoS, DoS, Reconnaissance, Brute Force, dan Benign. _Generalization gap_ dihitung sebagai selisih Macro F1 antara evaluasi _in-dataset_ dan _cross-dataset_ untuk masing-masing algoritma, sehingga model dengan degradasi performa terkecil dapat diidentifikasi sebagai algoritma paling _robust_ untuk skenario deteksi lintas domain IoT ke IIoT.
## 3.2 Desain Sistem
...
Kedua pipeline melalui empat tahap berurutan sebelum evaluasi: (1) *data processing*, (2) *feature selection* pada CICIoT2023 atau *feature alignment* pada DataSense, (3) *normalization*, lalu (4) *data splitting* dan evaluasi. Pipeline CICIoT2023 berakhir pada pembagian *stratified* 80:20 — 80% untuk pelatihan model dan 20% untuk evaluasi *in-dataset*. Pipeline DataSense tidak menjalani pembagian karena seluruh datanya difungsikan sebagai *test set* untuk evaluasi *cross-dataset*. Hasil kedua evaluasi kemudian digabungkan untuk menghitung *generalization gap*.

### 3.2.1 Data Processing

Pemrosesan data merupakan tahap pertama pada kedua pipeline dan bertujuan mengubah data mentah dari masing-masing dataset ke bentuk yang bersih dan siap untuk diproses lebih lanjut.

Pada pipeline **CICIoT2023**, pemrosesan data mencakup dua langkah berurutan. Pertama, *data cleaning*: menghapus baris duplikat, menangani nilai hilang (*missing values*) dengan strategi *drop-row*, dan menghapus kolom non-informatif seperti alamat IP sumber/tujuan, nomor *port*, dan *timestamp* yang tidak berkontribusi pada deteksi pola serangan. Kedua, *random undersampling*: mendistribusikan ulang kelas dengan mengambil 437.853 sampel dari setiap kelas — mengacu pada distribusi Dzaki et al. [11] — menurunkan total data dari 46 juta baris menjadi sekitar 3,5 juta baris. Undersampling dilakukan terhadap seluruh 8 kelas CICIoT2023; tiga kelas tanpa padanan di DataSense (Mirai, Spoofing, Web-based) baru dieliminasi pada tahap penyelarasan fitur sebelum evaluasi *cross-dataset*.

Pada pipeline **DataSense**, pemrosesan data mencakup *cleaning* yang setara: menghapus duplikat, menangani nilai hilang, dan menghapus kolom non-informatif. DataSense tidak menjalani *undersampling* karena digunakan seluruhnya sebagai *test set*.

### 3.2.2 Feature Selection dan Feature Alignment

Tahap kedua pada kedua pipeline menangani fitur dengan pendekatan yang berbeda sesuai peran masing-masing dataset.

Pada pipeline **CICIoT2023**, *Gini Impurity Tree-based feature selection* diterapkan untuk memilih subset fitur paling informatif dari 48 fitur yang tersisa setelah *cleaning*. Algoritma Decision Tree dengan kriteria Gini Impurity digunakan sebagai selektor, menghasilkan 10 fitur terpilih yang kemudian digunakan sebagai ruang fitur bersama ketiga model [11].

Pada pipeline **DataSense**, *feature alignment* dilakukan untuk menyelaraskan ruang fitur DataSense dengan 10 fitur terpilih dari CICIoT2023 — tanpa seleksi ulang — sehingga model menerima dimensi input yang identik di kedua domain. Pada tahap ini pula dilakukan pemetaan label untuk menyamakan basis kelas: 8 kelas CICIoT2023 dipetakan ke **5 kelas bersama** (DDoS, DoS, Reconnaissance, Brute Force, Benign), dengan kelas Mirai, Spoofing, dan Web-based pada CICIoT2023 serta kelas Man-in-the-Middle dan Malware pada DataSense dieliminasi karena tidak memiliki padanan langsung di dataset yang berlawanan.

### 3.2.3 Normalization

Setelah seleksi dan penyelarasan fitur, normalisasi Min-Max diterapkan pada kedua pipeline untuk menskalakan semua nilai fitur ke rentang [0,1]. Parameter normalisasi — nilai minimum dan maksimum per fitur — dihitung eksklusif dari data CICIoT2023 dan diterapkan secara konsisten pada DataSense tanpa menghitung ulang dari data *test set* [10][11]. Pendekatan ini memastikan transformasi yang konsisten lintas domain dan menghindari kebocoran informasi dari DataSense ke proses pelatihan.

### 3.2.4 Data Splitting, Training, dan Evaluation

Setelah normalisasi, pipeline CICIoT2023 dibagi menggunakan *stratified split* 80:20. Partisi 80% digunakan untuk melatih ketiga model secara independen, sementara partisi 20% digunakan sebagai *test set* evaluasi *in-dataset*. *Stratified split* diterapkan untuk memastikan distribusi kelas yang proporsional di kedua partisi.

Tiga model dilatih secara independen pada partisi *training* yang sama. **Decision Tree** dilatih dengan kriteria Gini Impurity sebagai model *baseline* pohon tunggal yang sepenuhnya *interpretable*. **Random Forest** dilatih menggunakan mekanisme *bootstrap aggregating* di mana sejumlah pohon keputusan dibangun secara paralel, kemudian prediksi digabung melalui *majority voting*. **XGBoost** dilatih menggunakan *gradient boosting* sekuensial dengan regularisasi L1 dan L2 bawaan, di mana setiap pohon baru difokuskan memperbaiki kesalahan residual model sebelumnya.
...
**Evaluasi *in-dataset*:** Ketiga model diaplikasikan pada partisi *test* 20% CICIoT2023. Untuk setiap model dihasilkan: *classification report* per kelas (Precision, Recall, F1-score), *confusion matrix* 8×8, Weighted AUC sebagai metrik agregat utama [26], Macro F1 sebagai metrik sekunder yang memberi bobot setara pada kelas minoritas [10], serta FPR per kelas sebagai ukuran alarm palsu.

**Evaluasi *cross-dataset*:** Model yang sama diaplikasikan pada seluruh DataSense — tanpa modifikasi atau *retraining* apapun — setelah pemetaan label ke 5 kelas bersama [26]. Output yang dihasilkan identik dengan evaluasi *in-dataset* namun pada skala 5 kelas.

**Perhitungan *Generalization Gap*:** Selisih performa antara kedua evaluasi dihitung untuk masing-masing model:
