---
jumlah references: "29"
rentang waktu penelitian: "2002-2026"
keyword:
  - IoT intrusion detection
  - IIoT security
  - machine learning
  - deep learning
  - CICIoT2023
  - DataSense
  - attack classification
  - feature selection
  - preprocessing
  - ensemble learning
list-website: researchrabbit.ai, scispace.com, perplexity.ai, semanticscholar.org

reading-hierarchy:
  instruction: >
    File ini adalah indeks bibliografi ringkas. Untuk informasi detail per paper
    (metodologi, desain eksperimen, hasil, kutipan spesifik), BACA file PDF-nya
    via path yang tercantum di setiap entri. Prioritaskan membaca PDF untuk paper
    yang paling relevan dengan pertanyaan yang diajukan.
  pdf-folder: "references/paper/"
  status-legend:
    "✅": "PDF tersedia dan dapat dibaca"
    "❌": "PDF tidak tersedia — gunakan URL atau cari manual"
    "⚠️": "PDF ada tapi 0 bytes — perlu re-download dari URL"
---

> **📖 Instruksi Baca Hierarkis**
> File ini adalah ringkasan bibliografi. Setiap entri mencantumkan path PDF-nya.
> Untuk detail lengkap (metodologi, hasil eksperimen, kutipan), **baca file PDF**
> yang ditunjuk sebelum menjawab pertanyaan spesifik tentang paper tersebut.
> Status: ✅ tersedia · ❌ tidak ada · ⚠️ 0 bytes (perlu re-download)

---

# Daftar Pustaka

<!-- KATEGORI: 2 Dataset Utama -->
## Dataset Utama

[1] Firouzi, A., Dadkhah, S., Maret, S. A., Ghorbani, A. A., DataSense: A Real-Time Sensor-Based Benchmark Dataset for Attack Analysis in IIoT with Multi-Objective Feature Selection, Electronics, Vol. 14, No. 20, Art. No. 4095, 2025.
https://www.mdpi.com/2079-9292/14/20/4095
> 📄 ✅ `references/paper/01_datasense-cic-iiot-2025.pdf`

[2] Neto, E. C. P., Dadkhah, S., Ferreira, R., Zohourian, A., Lu, R., Ghorbani, A. A., CICIoT2023: A Real-Time Dataset and Benchmark for Large-Scale Attacks in IoT Environment, Sensors, Vol. 23, No. 13, Art. No. 5941, 2023.
https://www.mdpi.com/1424-8220/23/13/5941
> 📄 ✅ `references/paper/02_CICIoT2023_Neto_Sensors_2023.pdf`

---

<!-- KATEGORI: 2 Penerapan ML pada IoT -->
## Penerapan ML pada IoT

[3] Alsamiri, J., Alsubhi, K., Internet of Things Cyber Attacks Detection Using Machine Learning, International Journal of Advanced Computer Science and Applications (IJACSA), Vol. 10, No. 12, 2019.
https://thesai.org/Downloads/Volume10No12/Paper_80-Internet_of_Things_Cyber_Attacks_Detection.pdf
> 📄 ✅ `references/paper/03_Alsamiiri_IoT_Cyber_Attacks_Detection_IJACSA_2019.pdf`

```note
implementasi steps nya oke banget, cuman gaperlu featuredari cic flow meter karena makan waktu, komparasi nya juga oke
```

[4] Churcher, A., Razdan, R., Ahmad, I., Tachtatzis, C., Sherwood, D., Weir, G., Andonovic, I., Michie, C., Tan, W. S., An Experimental Analysis of Attack Classification Using Machine Learning in IoT Networks, Sensors, Vol. 21, No. 2, Art. No. 446, 2021.
https://arxiv.org/pdf/2101.12270
> 📄 ✅ `references/paper/04_Churcher_Attack_Classification_IoT_Sensors_2021.pdf`

```note
penjelasan ids nya oke, dan poin 2.2 nya bisa dipake karena dia make algoritma ml utk deteksi iot network
```

---

<!-- KATEGORI: 1 Penerapan ML pada Dataset -->
## Penerapan ML pada Dataset

[5] Abebe, A., Gebeyehu, S., Alem, A., Internet of Things Attack Detection Using Machine Learning Algorithms, F1000Research, Vol. 14, Art. No. 230, 2026.
https://f1000research.com/articles/14-230
> 📄 ✅ `references/paper/05_Abebe_IoT_Attack_Detection_F1000_2026.pdf`

```note
dataset informasi nya bisa dipake, hasil evaluasinya jg bisa dipake 

implementasi nya sangat sederhana dan kayaknya pake ini aja deh 
```

---

<!-- KATEGORI: Dasar Teori IIoT -->
## Dasar Teori IIoT

[24] Alabadi, M., Habbal, A., Wei, X., Industrial Internet of Things: Requirements, Architecture, Challenges, and Future Research Directions, IEEE Access, Vol. 10, Hal. 66374–66400, 2022.
https://doi.org/10.1109/ACCESS.2022.3185049
> 📄 ❌ *PDF tidak tersedia — gunakan URL di atas*

[25] Gaber, T., Awotunde, J. B., Folorunso, S. O., Ajagbe, S. A., Eldesouky, E., Industrial Internet of Things Intrusion Detection Method Using Machine Learning and Optimization Techniques, Wireless Communications and Mobile Computing, Vol. 2023, Art. No. 3939895, Hal. 1–15, 2023.
https://onlinelibrary.wiley.com/doi/10.1155/2023/3939895
> 📄 ❌ *PDF tidak tersedia — gunakan URL di atas*

---

## Survey IoT/IIoT Intrusion Detection

[6] Bzai, J., Alam, F., Dhafer, A., Bojovic, M., Altowaijri, S. M., Niazi, I. K., Mehmood, R., Machine Learning-Enabled Internet of Things (IoT): Data, Applications, and Industry Perspective, Electronics, Vol. 11, No. 17, Art. No. 2676, 2022.
https://www.mdpi.com/2079-9292/11/17/2676
> 📄 ✅ `references/paper/06_Bzai_ML_Enabled_IoT_Electronics_2022.pdf`

[7] Blali, A., Dargaoui, S., Azrour, M., Guezzaz, A., Amounas, F., Alabdulatif, A., Analysis of Deep Learning-Based Intrusion Detection Systems in IoT Environments, EDPACS, Vol. 70, No. 11, Hal. 18–52, 2025
https://www.semanticscholar.org/paper/Analysis-of-deep-learning-based-intrusion-detection-Blali-Dargaoui/3f2a335a36bac8cfa6625d360cf4a0e9f3b746c6
> 📄 ❌ *PDF tidak tersedia — gunakan URL di atas*

```note
hasil eksperimennya bagus dan alurnya bisa dipake di proect

```

---

<!-- KATEGORI: 2 Teori ML General/Classification -->
## Teori ML General/Classification

[8] Ntayagabiri, J. P., Bentaleb, Y., Ndikumagenge, J., El Makhtoum, H., A Comparative Analysis of Supervised Machine Learning Algorithms for IoT Attack Detection and Classification, Journal of Computing Theories and Applications, Vol. 2, No. 3, Hal. 395–409, 2025.
https://dl.futuretechsci.org/id/eprint/101/1/11901-Article%20Text-43408-1-10-20250213.pdf
> 📄 ✅ `references/paper/08_Ntayagabiri_Supervised_ML_IoT_Attack_JCTA_2025.pdf`

```note
modelnya kebanyakan utk analisis dataset
```

[9] Saheed, Y. K., Abiodun, A. I., Misra, S., Holone, M. K., Colomo-Palacios, R., A Machine Learning-Based Intrusion Detection for Detecting Internet of Things Network Attacks, Alexandria Engineering Journal, Vol. 61, No. 12, Hal. 9395–9409, 2022.
https://www.sciencedirect.com/science/article/pii/S1877050922021470
> 📄 ✅ `references/paper/09_Saheed_ML_Intrusion_Detection_Alexandria_2022.pdf`

```note
mirip dgn paper 3 tapi dengan evlauasi lebih banyak chart
gap penelitian
bab 4 dengan hasil yang lebih detail
```
---

<!-- KATEGORI: 1 Rare Class / Imbalance / Augmentation -->
## Rare Class / Imbalance / Augmentation

[10] Firdaus, F. S., Hatta, P., Basori, B., Evaluating Ensemble Versus Non-Ensemble Machine Learning Performance with Preprocessing Techniques for IoT Intrusion Detection on CICIoT2023, Jurnal Teknik Informatika (JUTIF), Vol. 7, No. 1, Hal. 606–618, 2026.
> 📄 ✅ `references/paper/10_Firdaus_Ensemble_CICIoT2023_JUTIF_2026.pdf`

---

<!-- KATEGORI: 1 Feature Selection / Preprocessing -->
## Feature Selection / Preprocessing

[11] Dzaki, M. H., Nugraha, A., Luthfiarta, A., Riyanto, A. A. R., Novandian, Y. D., Accelerating Classification for IoT Attack Detection Using Decision Tree Model with Gini Impurity Tree-Based Feature Selection Technique, Jurnal Teknik Informatika (JUTIF), Vol. 6, No. 5, Hal. 3405–3418, 2025.
> 📄 ✅ `references/paper/11_Dzaki_Decision_Tree_Gini_JUTIF_2025.pdf`

[12] Sarhan, M., Layeghy, S., Portmann, M., Feature Analysis for Machine Learning-Based IoT Intrusion Detection, arXiv preprint arXiv:2108.12732, 2022.
> 📄 ✅ `references/paper/12_Sarhan_Feature_Analysis_IoT_arXiv_2021.pdf`

```note
dataset maybe...
```

---

<!-- KATEGORI: 2 Paper dengan Akurasi Model Tertinggi -->
## Paper dengan Akurasi Model Tertinggi

[13] Bensaoud, A., Kalita, J., Optimized Detection of Cyber-Attacks on IoT Networks via Hybrid Deep Learning Models, arXiv preprint arXiv:2502.11470, 2025.
https://arxiv.org/pdf/2502.11470
> 📄 ✅ `references/paper/13_Bensaoud_Hybrid_Deep_Learning_IoT_arXiv_2025.pdf`

[14] Nourildean, S. W., Ahmed, W. K., Sabri, A. A., Aziz, M., Intrusion Detection System Based Ensemble Machine Learning Model in AIoT Against Cyber Attacks, in Prosiding ASET 2025, Dubai, 2025, Hal. 1–7.
> 📄 ❌ *PDF tidak tersedia*

---

<!-- Tambahan: Ensemble Learning untuk IDS -->
## Ensemble Learning untuk IDS

[15] Premalatha, D. V., Ramanujam, S., Ensemble-Based Intrusion Detection for IoT Networks Using the CICIoT2023 Dataset, Journal of Information Systems Engineering and Management (JISEM), Vol. 10, No. 21s, 2025.
https://jisem-journal.com/index.php/journal/article/view/3407
> 📄 ✅ `references/paper/15_Premalatha_Ensemble_IDS_CICIoT2023_JISEM_2025.pdf`

```note
penjelasan tiap sistem design
```
---

<!-- KATEGORI: Survey IDS Berbasis ML -->
## Survey IDS Berbasis ML

[16] Gutierrez, J. A., Survey on Intrusion Detection Systems Based on Machine Learning Techniques for the Protection of Critical Infrastructure, Sensors, Vol. 23, No. 5, Art. No. 2415, 2023.
https://www.mdpi.com/1424-8220/23/5/2415
> 📄 ✅ `references/paper/16_Gutierrez_Survey_IDS_ML_Sensors_2023.pdf`

---

<!-- KATEGORI: SVM untuk IoT -->
## SVM untuk IoT

[17] Vassiliou, V., Network Attack Classification in IoT Using Support Vector Machines, Journal of Sensor and Actuator Networks, Vol. 10, No. 3, Art. No. 58, 2021.
https://www.mdpi.com/2224-2708/10/3/58
> 📄 ⚠️ `references/paper/17_Vassiliou_SVM_IoT_JSAN_2021.pdf` — *0 bytes, perlu re-download dari URL*

---

<!-- KATEGORI: Rare Class / Imbalance (tambahan) -->
## Rare Class / Imbalance / Augmentation (tambahan)

[18] Chawla, N. V., Bowyer, K. W., Hall, L. O., Kegelmeyer, W. P., SMOTE: Synthetic Minority Over-sampling Technique, Journal of Artificial Intelligence Research, Vol. 16, Hal. 321–357, 2002.
https://doi.org/10.1613/jair.953
> 📄 ❌ *PDF tidak tersedia — gunakan URL di atas*

[23] Alahmari, S., Aleisa, N., Enhancing the CIC IoT Dataset 2023 for Improved Attack Detection through GANs Augmentation and Federated Learning, Journal of Computer Science, Hal. 1688–1704, 2025.
https://thescipub.com/pdf/jcssp.2025.1688.1704.pdf
> 📄 ✅ `references/paper/23_Alahmari_CICIoT_GAN_Federated_JCS_2025.pdf`

```note
contoh gap penelitian
desain sistem
```

---

<!-- KATEGORI: Evaluasi Metrik Multiclass Imbalanced -->
## Evaluasi Metrik Multiclass Imbalanced

[19] Sokolova, M., Lapalme, G., A Systematic Analysis of Performance Measures for Classification Tasks, Information Processing & Management, Vol. 45, No. 4, Hal. 427–437, 2009.
https://doi.org/10.1016/j.ipm.2009.03.002
> 📄 ✅ `references/paper/19_Sokolova_Performance_Measures_Classification_IPM_2009.pdf`

---

<!-- KATEGORI: Cross-Dataset / Survey Generalisasi IDS -->
## Cross-Dataset / Survey Generalisasi IDS

[20] Khraisat, A., Gondal, I., Vamplew, P., Kamruzzaman, J., Survey of Intrusion Detection Systems: Techniques, Datasets and Challenges, Cybersecurity, Vol. 2, Art. No. 20, 2019.
https://cybersecurity.springeropen.com/articles/10.1186/s42400-019-0038-7
> 📄 ✅ `references/paper/20_Khraisat_Survey_IDS_Cybersecurity_2019.pdf`

[26] Farah, A., Cross-Dataset Evaluation for IoT Network Intrusion Detection, Master's Thesis, University of Wisconsin-Milwaukee, 2020.
https://minds.wisc.edu/server/api/core/bitstreams/c2baf164-6d13-4ee8-927b-3476fd493985/content
> 📄 ✅ `references/paper/26_Farah_Cross_Dataset_IDS_Thesis_2020.pdf`

```notes
gap penelitian
evaluation methodsnya bagus buat dipake
```

[27] Cantone, M., Marrocco, C., Bria, A., On the Cross-Dataset Generalization of Machine Learning for Network Intrusion Detection, IEEE Access, Vol. 12, Hal. 144489–144508, 2024.
https://doi.org/10.1109/ACCESS.2024.3472907
> 📄 ✅ `references/paper/27.pdf` — *nama file belum dilengkapi*

[29] Elangovan, R., Parthasarathy, D. D., Jawahar, M., Kaliyaperumal, P., Balusamy, B., Yogarajan, S., Venkatesan, V., Cross-Dataset Temporal and Semantic Generalization of Intrusion Detection Models for the Future Internet, 2026.
> 📄 ✅ `references/paper/29_Elangovan_CrossDataset_Temporal_Generalization_IDS_FutureInternet_2026.pdf`

```note
sangat relevan — evaluasi temporal robustness + cross-domain transfer (enterprise ke IoT)
membuktikan tree-based ensemble paling robust lintas dataset (degradasi 6–23%)
semantic feature stability: flow intensity & temporal activity lebih stabil lintas dataset
bagus banget buat bagian cross-dataset generalization gap
```

---

<!-- KATEGORI: Explainability / Feature Importance (SHAP) -->
## Explainability / Feature Importance (SHAP)

[21] Lundberg, S. M., Lee, S. I., A Unified Approach to Interpreting Model Predictions, Advances in Neural Information Processing Systems (NeurIPS), Vol. 30, Hal. 4765–4774, 2017.
https://proceedings.neurips.cc/paper/2017/hash/8a20a8621978632d76c43dfd28b67767-Abstract.html
> 📄 ✅ `references/paper/21_Lundberg_SHAP_NeurIPS_2017.pdf`

[22] Al-Shibly, I., Burgas, L., Massana, J., Interpretable Intrusion Detection for IoT: A CNN-BiLSTM Permutation Importance Framework for Deep Feature Selection, Frontiers in Big Data, Vol. 9, Art. No. 1813265, 2026.
https://www.frontiersin.org/journals/big-data/articles/10.3389/fdata.2026.1813265/full
> 📄 ✅ `references/paper/22_AlShibly_CNN_BiLSTM_Permutation_FrontiersBigData_2026.pdf`

```note
pake data terbaru, pake ini kalau mau contoh data 2025
gap penelitian
detail desain sistem
```

---

## references: mau ku tambahin,
